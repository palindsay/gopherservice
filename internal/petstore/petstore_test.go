...
func TestCreatePet(t *testing.T) {
	...
	pet := &v1.Pet{Id: "1", Name: "Fido"}
	req := &v1.CreatePetRequest{Pet: pet}
	res, err := s.CreatePet(ctx, req)
	assert.NoError(t, err)
	assert.Equal(t, pet, res.GetPet())

	// Test case 2: Attempt to create a pet with an empty ID, expecting an InvalidArgument error.
	req = &v1.CreatePetRequest{Pet: &v1.Pet{}}
